...
<template>
  <div class="grid gap-4">
    <template v-if="materialGroups.length" :key="expanded ? 'expanded' : 'collapsed'">
      <section v-for="group in materialGroups" :key="group.tier" class="grid gap-2.5">
        <div class="flex items-center justify-between gap-3">
          ...
          <div v-if="canExpandRecipes && group.tier === materialGroups[0]?.tier" class="flex items-center gap-2.5">
            ...
            <button
              type="button"
              class="relative h-5 w-9.5 rounded-pill border border-[rgba(122,144,184,0.22)] bg-[rgba(255,255,255,0.08)] p-0 transition-colors"
